...
import { GitBranch, Search, Check, X, Gamepad2, Wrench, RefreshCw, Eye } from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="pt-20 max-w-7xl mx-auto px-4 pb-12">
        ...
        <div className="flex gap-2 mb-6">
          {["listings", "posts"].map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black capitalize transition-all ${tab === t ? "bg-purple-600 text-white" : "bg-gray-900 border border-gray-700 text-gray-400 hover:border-purple-500/50"}`}>
              {t === "listings" ? <Gamepad2 className="w-4 h-4" /> : <Wrench className="w-4 h-4" />}
              {t} ({t === "listings" ? listings.length : posts.length})
            </button>
          ))}
        </div>
