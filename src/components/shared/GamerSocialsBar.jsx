...
export default function GamerSocialsBar({ className = "" }) {
  return (
    <div className={`flex flex-wrap items-center justify-start gap-2 overflow-visible opacity-100 px-3 sm:flex-nowrap sm:justify-center sm:overflow-x-auto sm:px-6 ${className}`}>
      <span className="text-[10px] font-black uppercase tracking-[0.25em] text-purple-300/80 mr-1 flex-shrink-0 w-full sm:w-auto">Follow Us</span>
      {SOCIALS.map((item) => {
        ...
        return (
          <a
            ...
            className="theme-glow-action inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-950/70 border border-purple-500/25 text-xs font-bold text-gray-300 hover:text-white transition-all whitespace-nowrap">
